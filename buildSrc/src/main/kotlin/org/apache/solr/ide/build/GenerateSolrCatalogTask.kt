...
abstract class GenerateSolrCatalogTask : DefaultTask() {
    ...
    private fun buildElements(line: String, version: String, jars: List<File>, scan: ScanResult): String {
        ...
        return buildString {
            ...
            appendLine("# name\tparent\tkind\tsources\tdiscontinued")
            SolrConfigElements.merge(fromPlugins + fromConfig + fromEditable).forEach {
                appendLine(
                    "${it.name}\t${it.parent}\t${SolrConfigElements.kindOf(it.arity)}\t${it.source}\t" +
    ...
    private fun buildCatalog(
        ...
        // Flattened to one sequence of rows before writing any, so that what a row is made of is
        // stated once. The three sources differ only in how they produce their (kind, class) pairs;
        // written as three loops, every new column was three edits with three chances to diverge.
        val rows = fieldTypes.map { "fieldType" to it } +
            scan.factories.toSortedMap().flatMap { (kind, implementations) ->
                implementations.sorted().map { kind to it }
            } +
            plugins.flatMap { (kind, implementations) -> implementations.map { kind to it } }

        return buildString {
            appendLine("# Generated by :generateSolrCatalog - do not edit.")
            appendLine("# Solr line $line, read from $version.")
            appendLine("# kind\tclass\tshort name\tattributes\tdocumentation\ttraits\tspi name")
            for ((kind, className) in rows) {
                appendCatalogRow(this, kind, className, hierarchy, documentation[className], scan.spiNames[className])
            }
        }
    }
    ...
    private fun appendCatalogRow(
        ...
        // Empty for every field type and plugin, which are named by class and never by an SPI short
        // name. Written for all kinds anyway, for the same reason `traits` is: one row shape.
        out.appendLine(
            "$kind\t$className\t${shortName(className)}\t$attributes\t$summary\t$traits\t${spiName.orEmpty()}",
        )
    }
...
private class ScanResult {
    ...
    // Class -> the SPI short name it registers under, for the classes that register one. Keyed by
    // the dotted name, because that is the form the catalog rows are written from. A schema may
    // name a component either way, and this is the half that cannot be derived: `lowercase` is
    // `LowerCaseFilterFactory` and `kStem` is `KStemFilterFactory`, and no single rule yields both.
    val spiNames = mutableMapOf<String, String>()
...
private class ArtifactScanner(private val analysisServices: Map<String, String>) {
    ...
    private fun visitClass(open: JarFile, entry: JarEntry, result: ScanResult) {
        open.getInputStream(entry).use { input ->
            ...
            val factory = reader.className.endsWith("Factory")
            // The SPI name lives in a `ConstantValue` attribute, so this needs no initializer walk —
            // the same reason the parameter constants are read by name above rather than by ancestry.
            // Stated inside the block below rather than beside it so the factory test is written once.
            if (factory || schemaClass) {
                if (factory) {
                    val constants = mutableListOf<Pair<String, String>>()
                    reader.accept(
                        ParameterConstantVisitor(constants),
                    ...
                    SolrSpiNames.of(constants)?.let {
                        result.spiNames[reader.className.replace('/', '.')] = it
                    }
                }
