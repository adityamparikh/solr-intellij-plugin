...
/**
 * Completes the attribute values and the vocabulary of a schema file, where the answers are knowable.
 *
 ...
 *
 * What can be completed inside a `solrconfig.xml` parameter is the other file's question, and lives
 * in
 * [SolrConfigCompletionContributor][org.apache.solr.ide.configset.solrconfig.completion.SolrConfigCompletionContributor].
 */
class SolrSchemaCompletionContributor : CompletionContributor() {
    ...
    init {
        ...
        extend(
            CompletionType.BASIC,
            PlatformPatterns.psiElement().inside(XmlPatterns.xmlTag()),
            SolrSchemaVocabularyCompletionProvider(),
        )
    }
}
