 ...
 *
 * A schema is held together entirely by string references — a field names its type, a `copyField`
 * names two fields, an analyzer component names a resource file — and none of them is a reference as
 * far as the editor is concerned. This is what turns them into one.
 *
 * The references a handler parameter makes are the same idea in the other file, and live in
 * [SolrConfigReferenceContributor][org.apache.solr.ide.configset.solrconfig.reference.SolrConfigReferenceContributor].
 * They are registered separately because a contributor is asked about every XML file in the project,
 * so the narrower its patterns the less it costs to decline.
 */
class SolrSchemaReferenceContributor : PsiReferenceContributor() {
