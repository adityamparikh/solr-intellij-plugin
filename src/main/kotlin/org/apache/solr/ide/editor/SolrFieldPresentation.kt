...
object SolrFieldPresentation {
    ...
    /**
     * The one-line hint shown inline beside a field, or null when nothing should be said.
     *
     * Null in two cases, both deliberate. An undeclared field type means the schema is wrong in a
     * way an inspection should report rather than a hint should paper over. An unconfident analysis
     * means a factory in the chain was not recognized, and a wrong claim about what a field matches
     * is worse than no claim.
     *
     * @param field the field to describe
     * @param fieldType its type, or null when the type it names is not declared
     * @return the hint text, or null to say nothing
     */
    fun inlayText(field: SolrField, fieldType: SolrFieldType?): String? {
        if (fieldType == null) return null
        val capability = SolrMatchAnalysis.of(fieldType)
        if (!capability.confident) return null
        return summarize(capability)
    }

    /**
     * The same summary as prose: granularity, case, and how partial matching is supported.
     *
     * @param capability the capability to describe
     * @return a comma-separated summary
     */
    fun summarize(capability: SolrMatchCapability): String = buildList {
        add(
            when (capability.granularity) {
                SolrMatchGranularity.WHOLE_VALUE -> "whole value"
                SolrMatchGranularity.TOKENS -> "tokenised"
            },
        )
        add(if (capability.caseSensitive) "case-sensitive" else "case-insensitive")
        when (capability.prefix) {
    ...
    fun fieldTypeDocumentation(fieldType: SolrFieldType, version: SolrVersionSelection): String = buildString {
        ...
        val capability = SolrMatchAnalysis.of(fieldType)
        if (capability.confident) {
            append("<p><b>Fields of this type match:</b> ${escape(summarize(capability))}.</p>")
            append(WILDCARD_CAVEAT)
        } else {
